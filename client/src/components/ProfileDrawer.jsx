...
const getExtras = (userId) => {
  try {
    const raw = localStorage.getItem('sysmagnet_profile_extras');
...
const setExtras = (userId, extras) => {
  try {
    ...
    const map = raw ? JSON.parse(raw) : {};
    map[userId] = extras;
    localStorage.setItem('sysmagnet_profile_extras', JSON.stringify(map));
  } catch { }
};
  ...
  const handleSave = async () => {
    if (user?.id) {
      try {
        ...
        if (updated) {
          try { window.dispatchEvent(new Event('sysmagnet_profile_update')); } catch {}
          // Switch user in AuthContext
          // Avoid circular import by calling onUpdate if provided; also directly patch localStorage
          try {
            const localRaw = localStorage.getItem('sysmagnet_user');
            ...
            localStorage.setItem('sysmagnet_user', JSON.stringify(nextUser));
          } catch {}
