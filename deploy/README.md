# Sysmagnet-CRM – VPS Deployment Guide (Supabase-only)
...
```
pm2 set pm2:sysmagnet-crm:SUPABASE_URL https://YOUR-PROJECT.supabase.co
...
```
sudo ln -s /path/to/deploy/nginx-example.conf /etc/nginx/sites-available/sysmagnet-crm
sudo ln -s /etc/nginx/sites-available/sysmagnet-crm /etc/nginx/sites-enabled/sysmagnet-crm
sudo nginx -t
